...
import {
  CounterState,
  createCounterText,
} from "../css/types/functions/counter";
import {
  LIST_STYLE_TYPE,
  listStyleType,
} from "../css/property-descriptors/list-style-type";
import {
...
export class DocumentCloner {
  ...
  createElementClone<T extends HTMLElement | SVGElement>(
    node: T
  ...
  createStyleClone(node: HTMLStyleElement): HTMLStyleElement {
    try {
      ...
      if (sheet && sheet.cssRules) {
        const css: string = [].slice
          .call(sheet.cssRules, 0)
          .reduce((css: string, rule: CSSRule) => {
    ...
    declaration.content.forEach((token) => {
      if (token.type === TokenType.STRING_TOKEN) {
        ...
        if (token.name === "attr") {
          ...
          const [counter, counterStyle] = token.values.filter(
          ...
          const [counter, delim, counterStyle] = token.values.filter(
            nonFunctionArgSeparator
          );
          if (counter && isIdentToken(counter)) {
        ...
        switch (token.value) {
          case "open-quote":
            anonymousReplacedElement.appendChild(
              document.createTextNode(
                getQuote(declaration.quotes, this.quoteDepth++, true)
              )
            );
            break;
          case "close-quote":
            anonymousReplacedElement.appendChild(
              document.createTextNode(
                getQuote(declaration.quotes, --this.quoteDepth, false)
              )
            );
            break;
          default:
...
const PSEUDO_HIDE_ELEMENT_CLASS_BEFORE =
