...
export class CanvasRenderer extends Renderer {
                ...
                switch (textDecorationLine) {
                  ...
                  case TEXT_DECORATION_LINE.LINE_THROUGH:
                    this.ctx.fillRect(
                      text.bounds.left,
                      text.bounds.top +
                        (text.bounds.height / 2 - decorationLineHeight / 2),
                      text.bounds.width,
                      decorationLineHeight
                    );
                    break;
  ...
  renderReplacedElement(
    ...
    const intrinsicWidth =
      (image as HTMLImageElement).naturalWidth || container.intrinsicWidth;
    const intrinsicHeight =
      (image as HTMLImageElement).naturalHeight || container.intrinsicHeight;
  ...
  async renderBackgroundImage(container: ElementContainer): Promise<void> {
    ...
    for (const backgroundImage of container.styles.backgroundImage
      ...
      if (backgroundImage.type === CSSImageType.URL) {
        ...
        if (image) {
          const imageWidth =
            isNaN(image.width) || image.width === 0 ? 1 : image.width;
          const imageHeight =
            isNaN(image.height) || image.height === 0 ? 1 : image.height;
        ...
        processColorStops(backgroundImage.stops, lineLength || 1).forEach(
          (colorStop) =>
