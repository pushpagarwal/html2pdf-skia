...
import {
  BACKGROUND_SIZE,
  BackgroundSizeInfo,
} from "../css/property-descriptors/background-size";
...
export const isAuto = (token: CSSValue): boolean =>
  isIdentToken(token) && token.value === BACKGROUND_SIZE.AUTO;

const hasIntrinsicValue = (value: number | null): value is number =>
  typeof value === "number";

export const calculateBackgroundSize = (
  size: BackgroundSizeInfo[],
  [intrinsicWidth, intrinsicHeight, intrinsicProportion]: [
    number | null,
    number | null,
    number | null
  ],
