...
export class ElementPaint {
  ...
  constructor(
    ...
    if (this.container.styles.overflowX !== OVERFLOW.VISIBLE) {
      ...
      if (equalPath(borderBox, paddingBox)) {
        ...
        this.effects.push(
          new ClipEffect(borderBox, EffectTarget.BACKGROUND_BORDERS)
        );
  ...
  getEffects(target: EffectTarget): IElementEffect[] {
    ...
    while (parent) {
      const croplessEffects = parent.effects.filter(
        (effect) => !isClipEffect(effect)
      );
      if (
        inFlow ||
...
const processListItems = (
  owner: ElementContainer,
  elements: ElementPaint[]
) => {
