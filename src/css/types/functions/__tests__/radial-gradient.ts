...
const parse = (value: string) =>
  radialGradient(
    {} as Context,
    (Parser.parseValues(value)[0] as CSSFunction).values
  );
const colorParse = (value: string) =>
  ...
  describe("radial-gradient", () => {
    describe("parsing", () => {
      it("radial-gradient(circle closest-side, #3f87a6, #ebf8e1, #f69d3c)", () =>
        deepStrictEqual(
          parse(
            "radial-gradient(ellipse closest-side, #3f87a6, #ebf8e1, #f69d3c)"
          ),
          {
      ...
      it("radial-gradient(circle at center, red 0, blue, green 100%)", () =>
        deepStrictEqual(
          parse("radial-gradient(circle at center, red 0, blue, green 100%)"),
          {
            type: CSSImageType.RADIAL_GRADIENT,
            ...
            stops: [
              {
                ...
                stop: { type: TokenType.NUMBER_TOKEN, number: 0, flags: 4 },
              },
