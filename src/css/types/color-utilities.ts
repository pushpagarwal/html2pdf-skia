...
export const isRelativeTransform = (tokens: CSSValue[]): boolean =>
  (tokens[0].type === TokenType.IDENT_TOKEN ? tokens[0].value : "unknown") ===
  "from";
...
export const packSrgbLinear = ([r, g, b, a]: [
  number,
  number,
  number,
  number
...
export const extractHslComponents = (
  context: Context,
  args: CSSValue[]
...
export const extractLchComponents = (
  args: CSSValue[]
): [number, number, number, number] => {
  const tokens = args.filter(nonFunctionArgSeparator),
...
export const extractLabComponents = (
  args: CSSValue[]
): [number, number, number, number] => {
  const tokens = args.filter(nonFunctionArgSeparator),
...
export const extractOkLchComponents = (
  args: CSSValue[]
): [number, number, number, number] => {
  const tokens = args.filter(nonFunctionArgSeparator),
    // eslint-disable-next-line prettier/prettier
...
const hsl2rgb = ([h, s, l]: [number, number, number]): [
  number,
  number,
  number
] => {
 ...
 * @param h
 */
const lch2lab = ([l, c, h]: [number, number, number]): [
  number,
  number,
  number
] => {
...
const lab2xyz = (lab: [number, number, number]): [number, number, number] => {
  const fy = (lab[0] + 16) / 116,
    ...
    xyz = [
      ...
      ((fz > e ? fz ** 3 : (116 * fz - 16) / k) * (1.0 - 0.3457 - 0.3585)) /
        0.3585,
    ];
