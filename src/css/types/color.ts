...
const _color = (context: Context, args: CSSValue[]) => {
  ...
  if (is_absolute) {
    ...
    if (typeof colorSpaceFunction === "undefined") {
      throw new Error(
        `Attempting to parse an unsupported color space "${color_space}" for color() function`
      );
    }
    ...
    if (typeof fromColorToXyz === "undefined") {
      throw new Error(
        `Attempting to parse an unsupported color space "${from_colorspace}" for color() function`
      );
    }
    if (typeof toColorFromXyz === "undefined") {
      throw new Error(
        `Attempting to parse an unsupported color space "${to_colorspace}" for color() function`
      );
    }

    const from_color: [number, number, number, number] = fromColorToXyz(
        context,
        from
      ),
      from_final_colorspace: [number, number, number, number] =
        toColorFromXyz(from_color),
