...
const metadataFolder = "../tmp/screenshots/metadata";
...
const writeScreenshot = (buffer: Buffer, body: ScreenshotRequest) => {
  const filename = `${filenamifyUrl(
    body.test.replace(/^\/tests\//, "").replace(/\.html$/, ""),
    {
      replacement: "-",
    }
  )}!${[
  ...
  fs.writeFileSync(
    path.resolve(__dirname, screenshotFolder, `${filename}.png`),
    buffer
  );
  return filename;
};
...
screenshotApp.post(
  "/screenshot",
  (
    req: express.Request<Record<string, never>, void, ScreenshotRequest>,
    ...
    const buffer = Buffer.from(
      req.body.screenshot.substring(prefix.length),
      "base64"
    );
